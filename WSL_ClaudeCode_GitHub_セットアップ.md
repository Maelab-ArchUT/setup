...
スタートメニューで「PowerShell」を検索 → 右側の「管理者として実行」をクリック。

![スタートメニューで PowerShell を検索し「管理者として実行」を選ぶ](images/powershell-search.png)

起動すると以下のような画面になります。

![管理者として起動した PowerShell の画面](images/powershell-admin.png)
...
- **New password**: 入力中は画面に文字もカーソルも表示されませんが、これは Linux の正常な仕様です (→ [パスワード入力中に何も表示されない](#パスワード入力中に何も表示されない))。そのまま打って Enter を押してください。
...
後続の `apt update` が `Network is unreachable` エラーで失敗するのを防ぐため、事前に IPv4 強制を設定しておきます (理由・対処の詳細は → [apt update が Network is unreachable で失敗する](#apt-update-が-network-is-unreachable-で失敗する))。
...
バージョンが返れば OK。`command not found` と出る場合は → [Claude Code の command not found](#claude-code-の-command-not-found)。
...
> **重要:** `sudo` を伴うコマンドは必ず Ubuntu のターミナルで直接実行してください。Claude Code セッション内で `!` から実行すると失敗します (→ [sudo: a terminal is required to read the password](#sudo-a-terminal-is-required-to-read-the-password))。
...
表示された 8 桁のコード (例: `1B93-DEFA`) をコピーしてから Enter。WSL ではブラウザが自動で開かず `Failed opening a web browser` と出ることが多いですが、これは正常で認証も失敗していません (→ [gh auth login でブラウザが開かない](#gh-auth-login-でブラウザが開かない))。

Windows 側のブラウザで手動で `https://github.com/login/device` を開き、コピーしておいたコードを貼り付けて認可してください。
...
「ファイル」→「フォルダーを開く」→ ダイアログのパス欄に `~/projects/test` と入力して開く
(セクション 5-3 で作成したフォルダ)。`~` は Linux 側ホームを指すので、ユーザー名を
打ち込む必要はありません。

別のフォルダを開きたい場合も、パス欄の先頭を `~/` から始めればホーム配下を指定できます。

### 7-4. VS Code 内で Claude Code を起動

VS Code のメニューから「ターミナル」→「新しいターミナル」を開きます (ショートカット: Ctrl + `)。
画面下部に開くターミナルは、WSL 接続中なら既に WSL 内のシェルです (7-2 の左下表示が
`WSL: Ubuntu-24.04` になっていることを確認)。そこで次を実行します。
...
セクション 5 で認証済みなので、再認証なしでそのまま起動します。左側でフォルダを開きながら、
このターミナルの Claude Code と対話する、というのが基本の作業スタイルです。

> **このマニュアルがカバーするのはここまで** — VS Code の統合ターミナルで `claude` が
> 起動する状態です。Claude の編集差分を VS Code 上に表示するなどの IDE 連携は、別途
> Claude Code の VS Code 拡張を入れると使えますが、必須ではありません。ターミナルの
> Claude Code だけで一通りの作業ができます。
...
本文の各所からこの付録の該当項目にリンクしています。困ったときはここを参照してください。

### パスワード入力中に何も表示されない

Ubuntu の初期設定や `sudo` のパスワード入力では、**入力中に文字もカーソルも一切表示されません**。これは Linux の正常な仕様で、フリーズではありません。そのまま打って Enter を押してください。
...
WSL2 の NAT ネットワークは IPv6 のルーティングが不安定で、`apt update` がこのエラーで失敗することがあります。対策はセクション 3-2 の IPv4 強制設定です。実施済みでも失敗する場合は DNS の問題の可能性があるので、`/etc/resolv.conf` を確認し、必要なら `nameserver 8.8.8.8` を追記してください。

### sudo: a terminal is required to read the password

Claude Code のセッション内で `!` プレフィックスから `sudo` を実行した場合に発生します。`sudo` を伴うコマンド（`apt install` など）は Claude Code の中ではなく、**別の WSL ターミナルを開いてそこで直接実行**してください。これが鉄則です。
...
root で入ると `passwd` だけでログインユーザーのパスワードを変更できます (ユーザー名を打つ必要はありません):

**Ubuntu (WSL / Linux 側、root)**
...
WSL では `xdg-open` 等の自動起動ツールが無いため、ブラウザの自動起動に失敗します (`Failed opening a web browser` / `executable file not found in $PATH`)。これは想定内の挙動で、認証自体は失敗していません。表示されるワンタイムコードをコピーし、Windows 側ブラウザで `https://github.com/login/device` を手動で開いて貼り付ければ問題なく完了します。
